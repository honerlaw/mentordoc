...
func (repo *DocumentDraftRepository) FindLatestAccessibleDraftForDocuments(userId string, documentIds []string) ([]shared.DocumentDraft, error) {
	...
	query := fmt.Sprintf("SELECT d1.id, d1.document_id, d1.name, d1.creator_id, d1.published_at, d1.retracted_at, d1.created_at, d1.updated_at, d1.deleted_at FROM document_draft d1 LEFT JOIN document_draft d2 ON (d1.document_id = d2.document_id AND d1.created_at < d2.created_at) WHERE d2.document_id IS NULL AND d1.retracted_at IS NULL AND d1.document_id in (%s) AND (d1.published_at IS NOT NULL OR (d1.published_at IS NULL AND d1.creator_id = ?))", placeholders);
